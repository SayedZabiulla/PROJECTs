...
from model_densenet import create_densenet_model
...
# Create directories
DENSENET_MODELS_PATH = os.path.join(MODELS_PATH, "densenet121")
DENSENET_RESULTS_PATH = os.path.join(RESULTS_PATH, "densenet121")
os.makedirs(DENSENET_MODELS_PATH, exist_ok=True)
os.makedirs(DENSENET_RESULTS_PATH, exist_ok=True)

# Training Configuration (SAME AS RESNET50 FOR FAIR COMPARISON)
CONFIG = {
    'batch_size': 16,
    'num_epochs': 50,
    'learning_rate': 1e-4,
...
print("="*60)
print("DENSENET121 TRAINING CONFIGURATION")
print("="*60)
...
def main():
    ...
    # Load data splits (SAME AS RESNET50)
    print("\n" + "="*60)
    print("LOADING DATA")
    print("="*60)
    ...
    for epoch in range(1, CONFIG['num_epochs'] + 1):
        ...
        if val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(DENSENET_MODELS_PATH, 'best_model_loss.pth')
            save_checkpoint(
        ...
        if val_metrics['f1_score'] > best_val_f1:
            ...
            best_model_f1_path = os.path.join(DENSENET_MODELS_PATH, 'best_model_f1.pth')
            save_checkpoint(
    ...
    # Save final model
    final_model_path = os.path.join(DENSENET_MODELS_PATH, 'final_model.pth')
    save_checkpoint(
        model, optimizer, scheduler, epoch, best_val_loss, history, final_model_path
    )
    
    # Save training history
    history_path = os.path.join(DENSENET_RESULTS_PATH, 'training_history.json')
    with open(history_path, 'w') as f:
        json.dump(history, f, indent=2)
    print(f"\n✓ Training history saved to: {history_path}")
    
    # Plot training history
    plot_path = os.path.join(DENSENET_RESULTS_PATH, 'training_history.png')
    plot_training_history(history, plot_path)
    
    # Save configuration
    config_path = os.path.join(DENSENET_RESULTS_PATH, 'training_config.json')
    ...
    print(f"Models saved in: {DENSENET_MODELS_PATH}")
    print(f"Results saved in: {DENSENET_RESULTS_PATH}")
